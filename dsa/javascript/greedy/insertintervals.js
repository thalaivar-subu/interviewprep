...
var insert = function (intervals, newInterval) {
    ...
    for (let i = 0; i < intervals.length; i++) {
        ...
        modifiedIntervals.push(intervals[i]);
    }
    if (!inserted) modifiedIntervals.push(newInterval);

    // Merge Intervals
    const result = [modifiedIntervals[0]]

    for (let i = 1; i < modifiedIntervals.length; i++) {
        let currentInterval = modifiedIntervals[i];
